...
rules(['first_name' => 'required', 'email' => 'required|email:rfc,dns', 'message' => 'required']);

with(fn() => [
    'options' => ContactUs::where('key', 'like', 'form-subject-option%')->get()->mapWithKeys(fn($content) => [$content->key => $this->isfrench ? $content->fr : $content->en]),
        ...
        <div>
            <form wire:submit.prevent="submit" class="font-light flex flex-col gap-6">
                <div class="flex max-xl:flex-col *:flex-1 gap-4">
                    <div>
                        <input wire:model="first_name" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-first-name-placeholder'])->striptags() !!}">
                        @error('first_name')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        <input wire:model="last_name" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-last-name-placeholder'])->striptags() !!}">
                        @error('last_name')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div>
                    <input wire:model="phone_no" type="number" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-phone-no-placeholder'])->striptags() !!}">
                    @error('phone_no')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    <input wire:model="email" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-email-placeholder'])->striptags() !!}">
                    @error('email')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                <div class="flex max-xl:flex-col *:flex-1 gap-4">
                    <div>
                        <input wire:model="company_name" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-company-name-placeholder'])->striptags() !!}">
                        @error('company_name')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        <input wire:model="job_title" class="bg-black/5 py-2 px-4 w-full outline-none" placeholder="{!! Str::of($content['form-job-title-placeholder'])->striptags() !!}">
                        @error('job_title')
                        <div class="text-red-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div>
                    <textarea wire:model="message" class="bg-black/5 py-2 px-4 w-full outline-none" rows="6" placeholder="{!! Str::of($content['form-message-placeholder'])->striptags() !!}"></textarea>
                    @error('message')
                    <div class="text-red-500 text-sm">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    <button wire:loading.class="*:last:invisible" wire:target="submit" class="relative uppercase py-2 px-6 bg-primary text-white cursor-pointer font-medium">
                        <div wire:loading.class.remove="invisible" wire:target="submit" class="absolute invisible inset-2 flex justify-center items-center">
                        ...
                        <div>
                            {!! Str::of($content['form-submit-button'])->striptags() !!}
                        </div>
                    </button>
                </div>
                <div>
